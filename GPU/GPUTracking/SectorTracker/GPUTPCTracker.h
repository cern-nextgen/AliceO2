...
class GPUTPCTracker : public GPUProcessor
{
 ...
 private:
  ...
  GPUglobalref() GPUAtomic(uint32_t) * mTrackletSortKeyCount = nullptr; // [NROWS*NROWS] histogram by (length, LastRow) key, later reused as prefix-sum offsets / scatter cursor
